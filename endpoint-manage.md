# Endpoint Management

This page is dedidicated to my Action1 endpoint management setup in my lab environment. Action 1 allows me to immediately identify vulnerabilites and install patches/updates.


&ensp;&ensp;&ensp;&ensp;

## Endpoint Dashboard

![Endpoint dashboard](media/endpoints.png)
![Endpoint dashboard](media/endpoints-2.png)

&ensp;&ensp;&ensp;&ensp;

## Automatic Patch/Updates

Updates on Windows Server


![Endpoint updates](media/endpoint-update.png)


Updates on Windows 11 Client

![Endpoint updates](media/endpoints-desktop2.png)

&ensp;&ensp;&ensp;&ensp;

## Testing remote access functionality

Remote access on Server
![Endpoint remote access](media/endpoint-remo.png)